...
def cmd_run_body(args, node, j):
    ...
    url_to_get_time=node.portal.url()+"/api/v2/jobs"+job.job_uid
    print("URL to find test start time:",url_to_get_time)
    f=open("ci_system/test_starttime_url.txt","w+")
    f.write(url_to_get_time)
    f.close()
    if args.mode == "pytest":
        for result in results:
